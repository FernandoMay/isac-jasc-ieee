...
\author{
    \IEEEauthorblockN{Fernando May Fuentes (费尔南多)}
    \IEEEauthorblockA{School of Information and Communication Engineering\\
    Beihang University (BUAA), Beijing, China\\
    LJ2502204\\
    fmayf@buaa.edu.cn\\
    Course: Principles of Communication and Communication Networks (通信和通信网络原理)\\
...
Figure~\ref{fig:tradeoff} presents the joint sensing-communication performance across the SNR range using $N_{\mathrm{MC}} = 10{,}000$ Monte Carlo realizations. The sensing prediction error decreases monotonically with SNR, from $\mathrm{PE}=0.047$ at SNR $=-15$~dB to $\mathrm{PE}=0.001$ at SNR $=+15$~dB. The communication BER follows the expected waterfall curve, achieving $\mathrm{BER}=8.2\times10^{-2}$ at SNR $=9$~dB and $5.1\times10^{-3}$ at SNR $=15$~dB. The inverse relationship between sensing and communication performance at low SNR ($<-10$~dB) reveals the fundamental trade-off: sensing requires higher SNR for reliable estimation due to the two-way propagation loss, while communication benefits from forward error correction and multipath diversity.
...
Figure~\ref{fig:se} shows the achievable spectral efficiency as a function of SINR at the communication receiver. The SE increases log-linearly with SINR, reaching $2.65$~bps/Hz at SINR $=+15$~dB after accounting for pilot overhead, cyclic prefix losses, and code rate. With MIMO extensions ($4\times4$ spatial multiplexing), the achievable SE scales to $>10$~bps/Hz. We overlay application-specific SE thresholds:

\begin{itemize}
    \item \textbf{Agriculture (4~bps/Hz):} Achieved at SINR $\approx +8$~dB in the SISO configuration, reachable at shorter ranges ($R < 150$~m) or with moderate antenna gain.
    \item \textbf{Finance (8~bps/Hz):} Requires MIMO or higher-order modulation, achievable with $2\times2$ MIMO at SINR $>+10$~dB.
\end{itemize}
...
Figure~\ref{fig:apps} compares the ISAC performance across two application scenarios representing agriculture monitoring (Feed the Future) and financial inclusion (GreenRemit). Agricultural scenarios, characterized by shorter ranges (50--150~m) and lower mobility ($|v|<5$~m/s), achieve higher spectral efficiency ($>1.2$~bps/Hz) and lower sensing error ($\mathrm{PE}<0.004$) compared to urban financial scenarios with longer ranges and higher mobility. The performance gap narrows with increasing SNR, suggesting that both applications are viable at SNR $>6$~dB. The CRLB analysis (Fig.~\ref{fig:crlb}) confirms that range estimation accuracy reaches sub-meter precision ($\sigma_R < 0.5$~m) for SNR $>+9$~dB, while velocity estimation achieves $\sigma_v < 5$~m/s across the entire SNR range.
...
Table~\ref{tab:results} presents the complete numerical results across the full SNR range, covering the low-SNR (challenging, PE$\sim$0.05), mid-SNR (operational, PE$<0.01$), and high-SNR (performance, PE$<0.002$) regimes. All values are computed from $N_{\mathrm{MC}} = 10{,}000$ independent Monte Carlo trials per configuration with random target positions ($R\sim\mathcal{U}[30,300]$~m), velocities ($v\sim\mathcal{U}[-20,20]$~m/s), and radar cross sections ($\sigma_{\mathrm{RCS}}\sim\mathcal{U}[0.1,10]$~m$^2$).
